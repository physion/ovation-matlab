% Load an HDF5 hierarchy
%
%   datastruct = hdf5load(filename)
%
%   Walks the group hierarchy of given h5 file and returns a
...
function datasets = hdf5load(filename)
	...
	datasets = struct;
    datasets = hdf5load_recursive(datasets,hinfo.GroupHierarchy);%%
...
function datastruct = hdf5load_recursive(datastruct, h5group)
	...
	for i = 1:numel(h5group.Groups)
		...
		groupname = strrep(groupname, ' ', '_');
		datastruct.(genvarname(groupname)) = hdf5load_recursive(struct(),h5group.Groups(i));%%
	end
