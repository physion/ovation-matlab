...
function hdf5save_recursive(filename,prefix,new,varcell)
	import ovation.*;
	...
	if ~strcmp(class(prefix),'char') ;
		error('second argument should be a string giving the name of the group to save the data in') ;
	end
		...
		switch type
			case 'struct'
				...
				for j=1:size(names,1) ;
					...
					hdf5save_recursive(filename,strcat(name,'/'),new,{names{j},varname});
				end
			otherwise
				location=strcat('/',prefix,name);
				if new==1 && i==1 ;
